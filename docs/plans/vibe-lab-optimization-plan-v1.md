...
- earlier Chronik, operator-routing, RepoBrief, rLens agent-context, Model-Lab and Operator-Lab work remains historical evidence and is not active merely because validators or files still exist;
...
- the validator inventory contains 95 classified validation targets: 45 core, 6 active-group and 44 legacy, plus two supplemental checks; the archived routing-readiness test frontdoor, the two Phase-1c archive frontdoors and the two never-executed rLens agent-context frontdoors are no longer blocking, while the six active-group targets retain current registry/registration or frozen Operator-Lab consumers;
...
5. **Consolidate validation.** The current inventory classifies 95 validation targets behind core, active and legacy frontdoors plus two supplemental checks; GitHub exposes a smaller CI presentation, and terminal historical frontdoors are retired only when their evidence remains reproducible or generic core coverage is proven.
...
- the never-executed rLens agent-context design and its two dedicated blocking frontdoors;
- any implied Heimlern, dashboard, Plexer, routing or runtime integration.

### Review for removal

The 44 remaining legacy targets remain blocking only until their protected family is archived or equivalent generic coverage is demonstrated. Review order:
...
4. historical PR-context validators and any remaining rLens-related surface only with separate current-consumer or archive evidence; the never-executed rLens agent-context design is already archived.
...
The legacy review date of 2026-09-01 has passed. Of the earlier 48-target baseline, 44 blocking legacy targets remain after the second evidence-backed survivor reduction. Every further target must receive one of three dispositions: `retain_with_consumer`, `covered_by_core` or `retire`; the directional reduction target never authorizes deletion without proof.
