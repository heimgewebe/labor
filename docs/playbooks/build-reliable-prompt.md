...
1. Als `prompts/adopted/<name>.md` ablegen; `consumer`, `decision_target` und eine `validated_by`-Relation auf die tragende Evidenz eines `adopted` + `executed|replicated` Experiments mit `adoption_assessment: adopt` im Frontmatter setzen und im Promotion-PR benennen
