...
3. **admit** — publiziere jeden natürlichen Fall vor Planung/Ausführung create-only:
   ...
   Ohne `--admissions-dir` schreibt der vorhandene Writer ausschließlich nach `experiments/<experiment>/artifacts/admissions/`. Der aktuelle Pfad verlangt eine **explizite, vor Planung belegte Condition-Zuteilung** im Request. Historische Registrierungen mit `assignment` bleiben Archivfakten; sie werden nicht automatisch fortgeführt, weil ohne extern verankerte Append-only-Allokationswahrheit keine belastbare Kohortenreihenfolge beweisbar wäre.
4. **observe** — binde die vorab definierte Messung mit `tools/vibe-cli/capture_effect_observation.py` an konkrete Evidenz. Für aktuelle, nicht grandfathered Experimente ist `--admission experiments/<experiment>/artifacts/admissions/<case-id>/admission.json` Pflicht; dadurch kann die Observation nicht von ihrer prospektiven Condition-Zuteilung getrennt werden. Die CLI verlangt außerdem Registration, Observations-Datei, Condition, Aufwand, Evidence-Ref/-Digest, Observer und Decision-Maker; `--help` zeigt den vollständigen Vertrag.
